...
u32 time_run=0;
u8  temp[33];
float Temp_pid[1];//用于PID计算采样的温度值
...
if (Time - TempCreatTrm >= TEMP_CREATTRM_INTERVAL)
  {
	...
	DS1820main();//采集一次温度数据存入TEMPA和B中已经转换为ASCII了直接写入文件系统中即可。。
  	...
  	#ifdef USED_SMTP
	  memcpy(&mailsentbox[0],&temp[6],5);//复制数据到IDbuff

	  memcpy(&mailsentbox[5],&temp[17],6);//复制数据到IDbuff

	  memcpy(&mailsentbox[11],&temp[24],6);//复制数据到IDbuff
	  mailsentbox[17] ='\0';
	#endif
