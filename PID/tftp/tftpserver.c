...
typedef struct
{
 ...
 //struct udp_pcb *Timeoutupcb; //超时PCB控制块
 //struct ip_addr *Timeoutaddr;
u16_t           Timeoutport;
struct pbuf     *Timeoutportpkt_buf;
...
char *tftp_errorcode_string[] = {
                                  "not defined/timeout-QQ:925295580",
                                  "file not found",
                                  "access violation",
                                  "disk full",
                                  "illegal operation",
...
void tftp_cleanup_wr(struct udp_pcb *upcb, tftp_connection_args *args)
{
  ...
  udp_recv(UDPpcb, recv_callback_tftp, NULL);
	...
	u8_t tftptimeoutTimer=0;//TFTP超时定时器
	u8_t tftptimeout=0;//TFTP超时定时器
                       ...
                       struct ip_addr *addr, u16_t port)
{
  ...
  {
    tftptimeoutTimer=0;//关闭TFTP超时定时器
	tftptimeout=0;
...
int tftp_process_read(struct udp_pcb *upcb, struct ip_addr *to, int to_port, char* FileName)
{
  ...
  tftptimeoutTimer=1;//开启TFTP超时定时器
...
void TftpTrm250ms(void) //250MS调用一次
{
  ...
  if( tftptimeoutTimer)//如果开启超时定时器则进入
  {
     tftptimeout++;
			  if(tftptimeout>85) //超过20S之后结束UDPPCB释放BUF。
			   {
